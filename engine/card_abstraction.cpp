...
static int sampled_flop_avg(const HandEvaluator& eval, Card cards[7], uint64_t used_mask,
                            const std::vector<Bucket>& river_table, int num_samples) {
...
Bucket CardAbstraction::get_bucket(Street street, Card hole0, Card hole1, const Card* board,
    ...
    switch (street) {
        ...
        case Street::FLOP: {
            ...
            int sum = sampled_flop_avg(eval, cards, used, rank_to_bucket_[2], FLOP_ROLLOUT_SAMPLES);
        ...
        case Street::TURN: {
            ...
            int bin = static_cast<int>(static_cast<double>(sum) / count * AVG_RESOLUTION + 0.5);
...
static void build_river_cdf_table(const HandEvaluator& eval, int nb, int num_threads,
    ...
    auto worker = [&](int tid) {
        ...
        while (true) {
            if (has_limit && combo_count.load(std::memory_order_relaxed) >= max_combos)
                break;
            int c0 = next_c0.fetch_add(1);
            if (c0 > 45)
                break;  // C(52,7): outermost 0..45
                            ...
                            for (int c5 = c4 + 1; c5 < 51; ++c5) {
                                ...
                                for (int c6 = c5 + 1; c6 < 52; ++c6) {
                                    ...
                                    if (has_limit && (++local_count & 0xFFFF) == 0) {
                                        ...
                                        if (combo_count.load(std::memory_order_relaxed) >=
                                            max_combos)
                                            return;
                                    }
                                }
    ...
    for (int i = 0; i < table_size; ++i) {
        if (freq[i] > 0) {
            cumulative += freq[i];
            Bucket b = static_cast<Bucket>(std::min(
                static_cast<uint64_t>(nb - 1), cumulative * static_cast<uint64_t>(nb) / total));
            out_table[i] = b;
            last_bucket = b;
        } else {
...
static void build_rollout_cdf_table(const HandEvaluator& eval, int n, int nb, int num_bins,
                                    int avg_resolution, int num_samples, int num_threads,
                                    const std::vector<Bucket>& river_table,
                                    std::vector<Bucket>& out_table, int64_t max_combos) {
    const bool has_limit = (max_combos > 0);

    std::vector<std::vector<uint64_t>> thread_freq(num_threads, std::vector<uint64_t>(num_bins, 0));
    std::atomic<int> next_c0{0};
    std::atomic<int64_t> combo_count{0};
                    ...
                    for (int c2 = c1 + 1; c2 < 50; ++c2) {
                        ...
                        for (int c3 = c2 + 1; c3 < 51; ++c3) {
                            ...
                            for (int c4 = c3 + 1; c4 < 52; ++c4) {
                                cards[4] = static_cast<Card>(c4);
                                uint64_t used = (1ULL << c0) | (1ULL << c1) | (1ULL << c2) |
                                                (1ULL << c3) | (1ULL << c4);
                                int sum =
                                    sampled_flop_avg(eval, cards, used, river_table, num_samples);
                                int bin = static_cast<int>(
                                ...
                                if (has_limit && (++local_count & 0x3FFF) == 0) {
                                    ...
                                    if (combo_count.load(std::memory_order_relaxed) >= max_combos)
                                        return;
                                }
                            }
                        }
...
void CardAbstraction::build_postflop_tables(const HandEvaluator& eval, int num_threads,
    ...
    {
        Timer t;
        build_river_cdf_table(eval, config::RIVER_BUCKETS, num_threads, rank_to_bucket_[2],
                              max_combos);
    ...
    // --- 2. Flop: sampled rollout (avg river percentile over 50 random turn+river) ---
    {
        ...
        log_info("  Flop (5 cards, sampled rollout x" + std::to_string(FLOP_ROLLOUT_SAMPLES) +
                 ") built in " + std::to_string(t.elapsed_seconds()) + "s");
    }
