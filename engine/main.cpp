...
int cmd_info() {
    ...
    if (std::filesystem::exists(config::CHECKPOINT_DIR)) {
        ...
        for (const auto& entry :
             std::filesystem::directory_iterator(config::CHECKPOINT_DIR)) {
            if (entry.path().extension() == ".bin") count++;
        }
        std::cout << "\nCheckpoints: " << count << " files in "
...
int cmd_build_abstraction() {
    ...
    std::string path =
        std::string(config::CHECKPOINT_DIR) + "/abstraction.bin";
    abs.save(path);
...
int cmd_query() {
    ...
    for (const auto& entry :
        ...
        if (filename.find("strategy_") == 0 &&
            entry.path().extension() == ".bin") {
            int iter = std::stoi(
                filename.substr(9, filename.size() - 13));
            if (iter > max_iter) {
                max_iter = iter;
    ...
    std::cout << "Loaded strategy from " << latest << "\n"
              ...
              << "Memory: " << store.memory_bytes() / (1024 * 1024)
              << " MB\n\n";
    ...
    while (std::getline(std::cin, line)) {
        ...
        try {
            ...
            if (data) {
                ...
                std::cout << "Average strategy (" << (int)data->num_actions
                          << " actions):\n";
...
int main(int argc, char* argv[]) {
    ...
    if (command == "build-abstraction") return cmd_build_abstraction();
    if (command == "train") return cmd_train();
    if (command == "query") return cmd_query();
