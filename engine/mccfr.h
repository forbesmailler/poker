...
class MCCFR {
   public:
    MCCFR(InfoSetStore& store, const CardAbstraction& card_abs, const ActionAbstraction& action_abs,
          const HandEvaluator& eval);

    // One iteration of external-sampling MCCFR
    // Returns utility for traversing_player
    double traverse(const GameState& state, int traversing_player, Rng& rng, int iteration);
