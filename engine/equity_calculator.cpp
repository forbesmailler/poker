...
float EquityCalculator::compute_equity(Card hole0, Card hole1, const Card* board, int num_board,
                                       const HandEvaluator& eval, int num_samples) const {
    ...
    for (int s = 0; s < num_samples; ++s) {
        ...
        while (true) {
            ...
            if (dead & card_bit(opp0))
                continue;
            opp1 = static_cast<Card>(rng.next_int(52));
            if (opp1 == opp0 || (dead & card_bit(opp1)))
                continue;
            break;
        }
        ...
        HandRank my_rank = eval.evaluate(hole0, hole1, full_board[0], full_board[1], full_board[2],
                                         full_board[3], full_board[4]);
        HandRank opp_rank = eval.evaluate(opp0, opp1, full_board[0], full_board[1], full_board[2],
                                          full_board[3], full_board[4]);
    ...
    return (static_cast<float>(wins) + 0.5f * static_cast<float>(ties)) / static_cast<float>(total);
}

EquityHistogram EquityCalculator::compute_histogram(Card hole0, Card hole1, const Card* board,
                                                    int num_board, const HandEvaluator& eval,
                                                    int num_bins, int num_samples) const {
    ...
    for (int s = 0; s < num_samples; ++s) {
        ...
        for (int o = 0; o < OPPONENT_SAMPLES; ++o) {
            ...
            HandRank my_rank = eval.evaluate(hole0, hole1, full_board[0], full_board[1],
                                             full_board[2], full_board[3], full_board[4]);
            HandRank opp_rank = eval.evaluate(opp0, opp1, full_board[0], full_board[1],
                                              full_board[2], full_board[3], full_board[4]);
        ...
        float equity = (static_cast<float>(sub_wins) + 0.5f * static_cast<float>(sub_ties)) /
...
float EquityCalculator::emd(const EquityHistogram& a, const EquityHistogram& b) {
